...
import { CircleAlert, SendHorizontal, X } from "lucide-react";
...
export function ChatAssistantPanel({
  ...
  const { isOpen, close } = useChatAssistant();
  ...
  return (
    <div
      role="dialog"
      aria-label={`Mora, assistente virtual do imóvel ${propertyName}`}
      aria-hidden={!isOpen}
      className={cn(
    ...
    >
      <header className="flex items-center gap-3 border-b border-stone-200 bg-brand-800 px-4 py-3.5 text-white">
        <Image
          src="/mora.png"
          alt="Mora, assistente virtual"
          width={40}
          height={40}
          className="size-10 shrink-0 rounded-full object-cover ring-2 ring-white/30"
        />
        <div className="min-w-0 flex-1">
          <p className="text-sm font-semibold">Mora</p>
          <p className="truncate text-xs text-white/80">
            Assistente virtual · {propertyName}
          </p>
        </div>
        <button
          type="button"
        ...
        {messages.length === 0 && (
          <div className="grid gap-3">
            <ChatBubble role="assistant">
              Olá! Eu sou a Mora, a assistente da sua estadia. Posso ajudar
              com a senha do Wi-Fi, regras da casa, horários e dicas da
              região. Como posso ajudar?
            </ChatBubble>
      ...
      >
        ...
        <button
          ...
          className="grid size-10 shrink-0 place-items-center rounded-xl bg-brand-800 text-white transition-colors hover:bg-brand-900 disabled:cursor-not-allowed disabled:bg-stone-300"
        >
